...
namespace wwwroot.QBAuth
{
    public class QBRequest
    {
        public static QBOnlineconfigurationModel AuObj = new QBOnlineconfigurationModel();
        //public static QBOParameters objStatus = new QBOParameters();
        ...
        ReRun:
            try
            {
                ...
                //string uri = string.Format(ConfigurationManager.AppSettings["qboBaseUrl"].ToString() + "query ?query={2}", ConfigurationManager.AppSettings["qboBaseUrl"].ToString(), AuObj.RealmID.ToString(), Query);
                // string url = ConfigurationManager.AppSettings["RequestUrl"].ToString();
        ...
        public static bool PerformRefreshToken(string refresh_token)
        {
            ...
            try
            {
                //Int32 ComID = Session["storeid"];
                //var obj = DB.QBOnlineConfigurations.SingleOrDefault(b => b.StoreId == ComID);
                //if (obj != null)
                //{
                //    obj.ClientId = QBRequest.AuObj.ClientId.ToString();
                //    obj.ClientSecretKey = QBRequest.AuObj.ClientSecretKey.ToString();
                //    obj.QBRefreshToken = AuObj.QBRefreshToken;
                //    obj.QBToken = AuObj.QBToken;
                //    DB.SaveChanges();
                //}
            }
            catch (Exception ex)
