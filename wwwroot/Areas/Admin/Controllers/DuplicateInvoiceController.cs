...
{
    public class DuplicateInvoiceController : Controller
    {
        // GET: Admin/DuplicateInvoice
        // GET: Admin/DashBoard
        ...
        private IEnumerable GetData(int SearchRecords = 0, int deptname = 0, string startdate = "", string enddate = "", string payment = "", string Store_val = "", string searchdashbord = "", string AmtMaximum = "0", string AmtMinimum = "0")
        {
            ...
            RtnData = (from data in db.tbl_Invoice_DuplicateInvoiceDashbord(start_date, end_date, payment, deptname, Convert.ToInt32(storeid), Convert.ToString(searchdashbord).TrimEnd(), Convert.ToDecimal(AmtMaximum), Convert.ToDecimal(AmtMinimum))
        ...
        public ActionResult GetDatapagescroll(int PageSize, int currentPageIndex, string rdcash, string rdcheck, string datval, string datendval, int deptnm, string searchdashbord = "", string AmtMaximum = "0", string AmtMinimum = "0")
        {
            ...
            RtnData = (from data in db.tbl_Invoice_DuplicateInvoiceDashbord(start_date, end_date, chk, deptnm, Convert.ToInt32(storeid), Convert.ToString(searchdashbord).TrimEnd(), Convert.ToDecimal(AmtMaximum), Convert.ToDecimal(AmtMinimum))
            ...
            TotalDataCount = (from data in db.tbl_Invoice_DuplicateInvoiceDashbord(start_date, end_date, chk, deptnm, Convert.ToInt32(storeid), "", Convert.ToDecimal(AmtMaximum), Convert.ToDecimal(AmtMinimum))
            ...
            RtnDatalistcount = (from data in db.tbl_Invoice_DuplicateInvoiceDashbord(start_date, end_date, chk, deptnm, Convert.ToInt32(storeid), "", Convert.ToDecimal(AmtMaximum), Convert.ToDecimal(AmtMinimum))
        ...
        public ActionResult Grid(string[] radio, string txtstartdate, string txtenddate, string DrpLstdept, string AmtMaximum = "0", string AmtMinimum = "0")
        {
            ...
            return RedirectToAction("Index", "DuplicateInvoice", new { IsBindData = 1, currentPageIndex = 1, orderby = "id", IsAsc = 0, PageSize = 20, SearchRecords = 1, Alpha = "", deptname = DrpLstdept, startdate = txtstartdate, enddate = txtenddate, payment = paymethod, Store_val = "", searchdashbord = "", AmtMaximum = AmtMaximum, AmtMinimum = AmtMinimum });
            //IEnumerable RtnData = null;
            //string storeid = GlobalStore.GlobalStore_id;
                ...
                if (Convert.ToInt32(exists) == 0)
                {
                    ...
                    if (ScanInvoice != null)
                    {
                        if (ScanInvoice.ContentLength > 0)
                        {
                            ...
                            {
                                var Status2 = "";
                                if (PostedData.RedairectFrom != "DuplicateInvoice")
                                {
