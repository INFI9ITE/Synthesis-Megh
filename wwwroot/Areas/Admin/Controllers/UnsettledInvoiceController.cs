...
namespace wwwroot.Areas.Admin.Controllers
{
    ...
    public class UnsettledInvoiceController : Controller
    {
        ...
        public static string scollinvoiceid;
        ...
        public ActionResult Grid(int IsBindData = 1, int currentPageIndex = 1, string orderby = "id", int IsAsc = 0, int PageSize = 20, int SearchRecords = 1, string Alpha = "", int deptname = 0, string startdate = "", string enddate = "", string payment = "", string Store_val = "", string searchdashbord = "")
        {
            ...
            {
                BindData = GetData(SearchRecords, deptname, startdate, enddate, payment, Store_val, searchdashbord).OfType<Invoice_Dashbord_Select>().ToList();
        ...
        private IEnumerable GetData(int SearchRecords = 0, int deptname = 0, string startdate = "", string enddate = "", string payment = "", string Store_val = "", string searchdashbord = "")
        {
            ...
            RtnData = (from data in db.Unsetteled_Invoice_Administrator(start_date, end_date, payment, deptname, Convert.ToInt32(storeid), searchdashbord, user_id)
        ...
        public ActionResult GetDatapagescroll(int PageSize, int currentPageIndex, string rdcash, string rdcheck, string datval, string datendval, int deptnm, string searchdashbord = "")
        {
            string chk = "";
            if (rdcash != "")
                chk = rdcash;
            ...
            RtnData = (from data in db.Unsetteled_Invoice_Administrator(start_date, end_date, chk, deptnm, Convert.ToInt32(storeid), searchdashbord, user_id)
            ...
            TotalDataCount = (from data in db.Unsetteled_Invoice_Administrator(start_date, end_date, chk, deptnm, Convert.ToInt32(storeid), "", user_id)
            ...
            RtnDatalistcount = (from data in db.Unsetteled_Invoice_Administrator(start_date, end_date, chk, deptnm, Convert.ToInt32(storeid), "", user_id)
        ...
        [HttpPost]
        public ActionResult Grid(string[] radio, string txtstartdate, string txtenddate, string DrpLstdept)
        {
            ...
            return RedirectToAction("Index", "UnsettledInvoice", new { IsBindData = 1, currentPageIndex = 1, orderby = "id", IsAsc = 0, PageSize = 20, SearchRecords = 1, Alpha = "", deptname = DrpLstdept, startdate = txtstartdate, enddate = txtenddate, payment = paymethod, Store_val = "", searchdashbord = "" });
            //IEnumerable RtnData = null;
            //string storeid = GlobalStore.GlobalStore_id;
        ...
        public ActionResult Edit(Invoice_Edit PostedData, int ID = 0, HttpPostedFileBase ScanInvoice = null, string[] Deptid = null, string[] Amt = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var exists = db.tbl_Invoice_exists_InNo_Edit_New(ID, PostedData.Invoice_Number, Convert.ToDateTime(PostedData.Invoice_Date), PostedData.Store_id, PostedData.Type_id, PostedData.Vendor_id).Select(x => x.Value).FirstOrDefault();
                if (Convert.ToInt32(exists) == 0)
                {
                    ...
                    if (ScanInvoice != null)
                    {
                        if (ScanInvoice.ContentLength > 0)
                        {
                            ...
                            {
                                var Status2 = "";
                                if (PostedData.RedairectFrom != "UnsettledInvoice")
                                {
                    ...
                    if (PostedData.Discounttype != 1 && PostedData.Discounttype != null)
                    {
                        ...
                        else
                        {
                            ...
                            // if (System.IO.File.Exists(ConfigurationManager.AppSettings["cloudurl"] + "\\scanimage\\" + Invoice_data.ScanInvoice))
                            {
                                //string DSacn_Title1 = AdminSiteConfiguration.GetRandomNo() + Path.GetFileName(Invoice_data.ScanInvoice);
                                //DSacn_Title1 = AdminSiteConfiguration.RemoveSpecialCharacter(DSacn_Title1);
                        ...
                        if (!string.IsNullOrEmpty(sucess1))
                        {
                            ...
                            ActivityLogMessage = "Invoice Number " + "<a href='/Admin/InvoiceReport/Detail/" + ID + "'>" + PostedData.Invoice_Number + "_cr" + "</a> created by " + fullname + " on " + wwwroot.Class.AdminSiteConfiguration.GetDate(DateTime.Now.AddHours(1).ToString());
